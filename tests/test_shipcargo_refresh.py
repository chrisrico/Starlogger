"""The catalog-refresh decision logic in shipcargo.py: when each cached catalog (ship
...
Run: python3 -m pytest tests/test_shipcargo_refresh.py
"""
...
def _cat(label, has_cache, cached_ver, rebuild=None):
    return shipcargo._Catalog(label, lambda: has_cache, lambda: cached_ver,
                              rebuild or (lambda p4k, ver, reason: None))


def test_reason_no_cache():
    assert shipcargo._reason(_cat("x", False, None), "4.8") == "no cache"


def test_reason_version_moved():
    assert shipcargo._reason(_cat("x", True, "4.7"), "4.8") == "version 4.7 -> 4.8"
    assert shipcargo._reason(_cat("x", True, None), "4.8") == "version ? -> 4.8"


def test_reason_fresh_is_none():
    assert shipcargo._reason(_cat("x", True, "4.8.0"), "4.8.3") is None  # same major.minor
    assert shipcargo._reason(_cat("x", True, "4.7"), None) is None       # no live version yet


def test_refresh_once_rebuilds_only_stale(monkeypatch):
    monkeypatch.setattr(shipcargo.scdata, "find_p4k", lambda lp: "/fake/Data.p4k")
    calls = []
    ...
    shipcargo._refresh_once(cats, "4.8", None)
    assert calls == ["reference", "mineables"]


def test_refresh_once_no_p4k_skips_all(monkeypatch):
    monkeypatch.setattr(shipcargo.scdata, "find_p4k", lambda lp: None)
    calls = []
...
def test_refresh_once_failure_isolated(monkeypatch):
    ...
    shipcargo._refresh_once(cats, "4.8", None)   # 'a' raises, must not stop 'b'
    assert calls == ["b"]
